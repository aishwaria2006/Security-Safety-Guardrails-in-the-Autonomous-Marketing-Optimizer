...
class OptimizedCRMAgent:
    ...
    def __init__(self, config=CRMConfig):
        ...
        if Groq:
            try:
                ...
                print("[WARN]  LLM initialization failed")
        
        os.makedirs(config.MODEL_PATH, exist_ok=True)
        os.makedirs(config.DASHBOARD_EXPORT_PATH, exist_ok=True)
        
        print("[AGENT] Optimized CRM Agent initialized!")
    ...
    def batch_score_leads_optimized(self, df):
        ...
        print(f"\n[START] Batch scoring {len(df)} leads (OPTIMIZED)...")
        start_time = time.time()
        ...
        elapsed = time.time() - start_time
        print(f"[OK] Scored {len(df)} leads in {elapsed:.2f}s ({len(df)/elapsed:.0f} leads/sec)")
...
if __name__ == '__main__':
    print("="*80)
    print("[START] OPTIMIZED AI CRM AGENT - FLASK SERVER")
    print("="*80)
