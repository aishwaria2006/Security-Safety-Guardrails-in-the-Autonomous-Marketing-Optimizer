...
GMAIL_USER = os.getenv("GMAIL_USER", "your-email@example.com")
...
def send_email(to_email, subject, content, customer_name="Customer"):
    ...
    try:
        ...
        print(f"[FAIL] Email failed for {to_email}: {str(e)}")
        return False
...
def send_sms(to_phone, message, customer_name="Customer"):
    ...
    try:
        ...
        print(f"[OK] SMS sent to {to_phone} (SID: {message_obj.sid})")
        return True
        
    except Exception as e:
        print(f"[FAIL] SMS failed for {to_phone}: {str(e)}")
        return False
...
def initialize_llm():
    ...
    if GOOGLE_API_KEY and len(GOOGLE_API_KEY) > 20:
        try:
            ...
            print("[OK] Using Gemini")
            return llm, "gemini"
        except: pass
    print("[WARN] Using template fallbacks")
...
def generate_sample_data():
    ...
    try:
        if session.query(Customer).count() > 0:
            print("[OK] Data exists")
            session.close()
            return
        print("[DATA] Generating data...")
...
def agent_send_campaign(campaign_id):
    ...
    # Guardrail check: screen content for prompt injection
    from guardrails import screen_external_input, guardrail_manager
    screen_res = screen_external_input("campaign_content", c.content or "")
    if screen_res["flagged"]:
        session.close()
        log_agent_activity("System", "campaign_send", f"Campaign send blocked: Prompt injection detected", "failed")
        return {"status": "error", "message": f"Campaign content failed safety checks: prompt injection detected."}

    # Guardrail check: request action allowlist check
    # 50 customers * $25.00 cost = $1250, which exceeds standard $1000 agent cap to demonstrate the budget guard
    send_cost = float(len(customers) * 25.0)
    action_res = guardrail_manager.request_agent_action(
        agent_name="CampaignGenerator",
        action="send_campaign",
        params={"campaign_id": campaign_id, "amount": send_cost}
    )
    if action_res.get("status") == "pending_human_approval":
        session.close()
        log_agent_activity("System", "campaign_send", f"Campaign send paused: Pending human approval", "pending")
        return {"status": "pending_approval", "message": "Campaign send is pending human approval gate."}
    elif action_res.get("status") == "denied":
        session.close()
        log_agent_activity("System", "campaign_send", f"Campaign send blocked: {action_res.get('reason')}", "failed")
        return {"status": "denied", "message": f"Campaign send blocked: {action_res.get('reason')}"}
        
    print(f"\n{'='*60}")
    print(f"? SENDING CAMPAIGN: {c.name}")
    print(f"[DATA] Channel: {c.channel}")
    ...
    for customer in customers:
        ...
        if c.channel == 'email':
            ...
            print(f"??  Social media posting not implemented for {customer.name}")
            success = True
    ...
    print(f"\n{'='*60}")
    print(f"[OK] CAMPAIGN COMPLETE!")
    print(f"? Sent: {sent_count}")
    print(f"[OK] Delivered: {delivered_count}")
...
if __name__ == '__main__':
    print("\n" + "="*80)
    print("[START] ENHANCED AGENTIC CAMPAIGN SYSTEM")
    print("="*80)
    print("\n[EMAIL] Email Setup:")
    print(f"   Gmail: {GMAIL_USER}")
    print(f"   Status: {'[OK] Configured' if len(GMAIL_APP_PASSWORD) > 10 else '[FAIL] Not configured'}")
    print("\n? SMS Setup:")
    print(f"   Twilio SID: {TWILIO_ACCOUNT_SID[:10]}..." if len(TWILIO_ACCOUNT_SID) > 10 else "   [FAIL] Not configured")
    print(f"   Status: {'[OK] Configured' if len(TWILIO_AUTH_TOKEN) > 10 else '[FAIL] Not configured'}")
    print("\n[AGENT] AI Setup:")
    print(f"   Groq API: {'[OK] Configured' if len(GROQ_API_KEY) > 20 else '[FAIL] Not configured'}")
